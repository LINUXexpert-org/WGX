...
# Includes: optional Caddy Basic Auth, sanitized Caddyfile (no plugin auth/import),
# autosave purge, UFW, unattended-upgrades (codename placeholder fixed),
# fail2ban (Caddy HTTP errors + SSH), loopback-only wg-easy UI binding,
# systemd autostart, progress percentage bar, full log.
#
...
# ===== I/O helpers (FD3 for user; /dev/tty for reads) =====
...
# ===== Subnet normalization: "10.8.0.x" OR "10.8.0.0/24" -> address "10.8.0.x" + range =====
norm_cidr(){
...
# Normalize CIDR -> WG_DEFAULT_ADDRESS + RANGE
...
# ===== Optional BasicAuth bcrypt for Caddy (if enabled) =====
BASIC_HASH=""
...
# 5) Caddy install + strong drop-in + log perms + autosave purge
...
# 6) wg-easy .env + compose (bcrypt with htpasswd; single PASSWORD_HASH line; loopback bind)
run_step "Create wg-easy deployment files" "
mkdir -p '$CONFIG_DIR'

# Generate bcrypt using apache2-utils for maximum compatibility
WGEASY_PASS_HASH=\$(htpasswd -nbB user \"$WGEASY_ADMIN_PASS\" | cut -d: -f2 | tr -d '\r\n')
if ! printf '%s' \"\$WGEASY_PASS_HASH\" | grep -Eq '^\$2[aby]\$[0-9]{2}\$[./A-Za-z0-9]{53}$'; then
  echo 'Failed to generate a valid bcrypt for PASSWORD_HASH' >&2; exit 1
fi

# Fresh .env to guarantee exactly one PASSWORD_HASH
cat > '$APP_DIR/.env' <<EOF
WG_HOST=$WG_HOST
...
[ \$(grep -c '^PASSWORD_HASH=' '$APP_DIR/.env') -eq 1 ] || { echo 'Unexpected multiple PASSWORD_HASH lines in .env' >&2; exit 1; }

# Compose ensures UI listens on ${WG_EASY_PORT} and maps 127.0.0.1:${WG_EASY_PORT} -> container ${WG_EASY_PORT}
...
# 7) Caddyfile + sanitize + validate + handle plugin 'authentication' fallback
...
run_step "Write Caddyfile; sanitize & validate (fallback if plugin 'authentication')" "
cat > '$CADDYFILE' <<EOF
{
  admin off
  email $LE_EMAIL
...
# Remove plugin 'authentication' blocks/imports if present
awk 'BEGIN{skip=0}
...
# If plugin auth persists, try legacy basicauth; if still persists, drop web auth entirely
...
# 9) unattended-upgrades (keep ${distro_codename} literal)
...
# 13) Reload Caddy (force-compatible), purge autosaves again
